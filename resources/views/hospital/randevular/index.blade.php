...
@section('subtitle', 'Yalnızca bu kurumdaki doktorların randevuları · sayfa başına 30')

@section('content')
    <div class="mx-auto max-w-5xl">
    <div class="rounded-3xl border border-sky-100/80 bg-white/70 hospital-glass p-5 shadow-sm">
        <form method="get" action="{{ route($kr.'.randevular.index') }}" class="flex flex-wrap items-end justify-end gap-2">
                <div>
                            ...
                            <td class="px-3 py-3 text-right">
                                <div class="flex flex-wrap items-center justify-end gap-1.5">
                                    @if ($a->durum !== \App\Enums\RandevuDurumu::Tamamlandi)
                                        <form method="post" action="{{ route($kr.'.randevular.tamamla', $a) }}" class="inline">
                                            @csrf
                                    ...
                                    @endif
                                    <form method="post" action="{{ route($kr.'.randevular.destroy', $a) }}" class="inline" onsubmit="return confirm('Bu randevuyu kalıcı olarak silmek istediğinize emin misiniz? Saat müsait hale getirilecek.');">
                                        @csrf
                                        <input type="hidden" name="durum" value="{{ $durumFilter }}">
                                        @method('DELETE')
