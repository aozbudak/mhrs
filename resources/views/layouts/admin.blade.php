...
<body class="min-h-screen hospital-bg antialiased text-slate-900"
    ...
    <div class="flex min-h-screen">
        <aside id="admin-sidebar"
               class="fixed inset-y-0 left-0 z-40 flex w-64 max-w-[min(100vw-3rem,16rem)] flex-col border-r border-sky-200/60 bg-white/85 hospital-glass shadow-xl transition-transform duration-200 ease-out -translate-x-full lg:static lg:translate-x-0 lg:max-w-none lg:shadow-none">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4" aria-label="Admin menü">
                <a href="{{ route('admin.panel') }}"
                   class="admin-nav-item flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-semibold transition {{ request()->routeIs('admin.panel') ? 'bg-sky-100 text-sky-950 shadow-sm' : 'text-slate-600 hover:bg-white/70' }}">
                ...
                <a href="{{ route('admin.randevular.index') }}"
                   class="admin-nav-item flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-semibold transition {{ request()->routeIs('admin.randevular.*') ? 'bg-sky-100 text-sky-950 shadow-sm' : 'text-slate-600 hover:bg-white/70' }}">
                ...
                <a href="{{ route('admin.doktorlar.index') }}"
                   class="admin-nav-item flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-semibold transition {{ request()->routeIs('admin.doktorlar.*') ? 'bg-sky-100 text-sky-950 shadow-sm' : 'text-slate-600 hover:bg-white/70' }}">
            ...
            <div class="shrink-0 border-t border-sky-100/80 p-3">
                <div class="rounded-2xl border border-sky-100 bg-white/60 px-3 py-2.5">
                    <div class="truncate text-xs font-bold text-slate-800">{{ auth('admin')->user()?->name }}</div>
                    <div class="truncate text-[11px] text-slate-500">{{ auth('admin')->user()?->email }}</div>
        ...
        <div class="flex min-h-screen min-w-0 flex-1 flex-col">
            <header class="sticky top-0 z-20 flex h-16 shrink-0 items-center gap-3 border-b border-sky-100/80 bg-white/75 hospital-glass px-4 shadow-sm backdrop-blur-sm lg:px-6">
