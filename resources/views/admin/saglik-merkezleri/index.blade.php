...
@section('title', 'Sağlık Merkezleri')
@section('subtitle', 'Sistemdeki sağlık merkezi kayıtlarını yönetin')

@section('content')
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <form method="get" action="{{ route('admin.saglik-merkezleri.index') }}" class="flex flex-wrap items-end gap-2">
            <div>
            ...
            @if($filterCity)
                <a href="{{ route('admin.saglik-merkezleri.index') }}" class="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition">
                    Filtreyi kaldır
                </a>
            @endif
        </form>
        <a href="{{ route('admin.saglik-merkezleri.create') }}" class="inline-flex items-center justify-center rounded-2xl border border-emerald-200 bg-emerald-50/90 px-4 py-2 text-sm font-semibold text-emerald-900 hover:bg-emerald-100/90 transition">
            Yeni sağlık merkezi ekle
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="bg-sky-50/60 text-xs font-bold text-slate-700">
                    <tr>
                        <th class="px-3 py-2 text-left">Kurum adı</th>
                        ...
                        <tr class="hover:bg-sky-50/40 transition">
                            ...
                            <td class="px-3 py-3 text-right">
                                <div class="flex flex-wrap items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.saglik-merkezleri.edit', $hastane) }}" class="rounded-xl border border-sky-200 bg-sky-50/80 px-2.5 py-1.5 text-[11px] font-semibold text-sky-900 hover:bg-sky-100/80 transition">
                                        Düzenle
                                    </a>
                                    <form method="post" action="{{ route('admin.saglik-merkezleri.destroy', $hastane) }}" class="inline" onsubmit="return confirm('Bu sağlık merkezi kaydını silmek istediğinize emin misiniz?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-sm text-slate-600">
                                Henüz sağlık merkezi kaydı yok. «Yeni sağlık merkezi ekle» ile oluşturabilirsiniz.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
