...
@section('title', 'Sağlık merkezi düzenle')
    ...
    <div class="mx-auto max-w-5xl space-y-8">
        <form method="post" action="{{ route('admin.saglik-merkezleri.update', $hastane) }}" class="rounded-3xl border border-sky-100/80 bg-white/70 hospital-glass p-5 shadow-sm space-y-6">
            ...
            <div class="border-t border-sky-100 pt-6">
                <h2 class="text-sm font-extrabold text-slate-900">Kurum çalışma saatleri</h2>
                <p class="mt-1 text-xs text-slate-600 leading-relaxed">
                    Değişiklik kaydedildiğinde bu kurumdaki tüm doktorlar için bağlı randevusu olmayan gelecekteki boş slotlar silinir ve yeni programa göre yeniden üretilir.
                </p>
                <div class="mt-4">
        ...
        <div class="rounded-3xl border border-emerald-100/80 bg-white/70 hospital-glass p-5 shadow-sm space-y-4">
            ...
            <p class="text-xs text-slate-600">Yeni kullanıcı hesabı oluşturulur ve doğrudan bu kuruma atanır.</p>
        ...
        <div class="rounded-3xl border border-violet-100/80 bg-white/70 hospital-glass p-5 shadow-sm space-y-4">
            ...
            <p class="text-xs text-slate-600 leading-relaxed">
                Bu sa?l?k merkezi i�in <strong class="font-semibold text-slate-800">/hastane</strong> adresinden giri� yapabilecek bir hesap olu�turur. Giri� sayfas?nda ?Kurum? sekmesini se�melidir.
            </p>
            <form method="post" action="{{ route('admin.saglik-merkezleri.kurum-yoneticisi.store', $hastane) }}" class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label for="kurum_admin_password" class="text-xs font-bold text-slate-700">�ifre</label>
                    <input type="password" name="kurum_admin_password" id="kurum_admin_password" required autocomplete="new-password"
                           class="mt-1 w-full rounded-2xl border border-violet-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm" />
                </div>
                <div>
                    <label for="kurum_admin_password_confirmation" class="text-xs font-bold text-slate-700">�ifre (tekrar)</label>
