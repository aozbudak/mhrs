...
@section('subtitle', 'Giriş hesabı ve birim ataması oluşturulur')
    ...
    <div class="mx-auto max-w-2xl">
        ...
        @if($departments->isEmpty())
            <div class="rounded-3xl border border-amber-200 bg-amber-50/90 p-4 text-sm text-amber-950">
                Önce en az bir aktif birim tanımlanmalı. Veritabanında <code class="rounded bg-white/60 px-1">departments</code> tablosuna kayıt ekleyin veya seeder çalıştırın.
            </div>
        @endif

        <form method="post" action="{{ route('admin.doktorlar.store') }}" class="rounded-3xl border border-sky-100/80 bg-white/70 hospital-glass p-5 shadow-sm space-y-4">
            @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label for="password" class="text-xs font-bold text-slate-700">Şifre</label>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="tc_kimlik_no" id="tc_kimlik_no" value="{{ old('tc_kimlik_no') }}" maxlength="11" pattern="[0-9]{11}"
                           class="mt-1 w-full rounded-2xl border border-sky-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm" />
                    <p class="mt-1 text-[11px] text-slate-500">Boş bırakılırsa sistem benzersiz bir numara üretir.</p>
                </div>
                <div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Örn. Uzman Dr."
                           class="mt-1 w-full rounded-2xl border border-sky-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm" />
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="bio" id="bio" rows="3"
                          class="mt-1 w-full rounded-2xl border border-sky-200 bg-white px-4 py-2.5 text-sm text-slate-900 shadow-sm">{{ old('bio') }}</textarea>
            </div>
